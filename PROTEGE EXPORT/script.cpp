...
int main(){
    ...
    } else { cout << "Archivo classes.pont no encontrado o no ha podido abrirse." << endl; exit(1); } 
    loadFile.close();
    ...
    if (loadFile.is_open()) {
        for(int i=0; i<=3; ++i) getline(loadFile, linea);
        while (getline(loadFile, linea)) outputFile << "\t" + linea + "\n";
        cout << "Archivo instances.pins leido correctamente." << endl;
    } else { cout << "Archivo instances.pins no encontrado o no ha podido abrirse."; exit(1); }
    loadFile.close();
    outputFile << ")" << endl;
    outputFile.close();
} 
